 ...
 *
 */
public class MecanumDrive extends DeviceBase {
...
//    public void initMotor("")
    ...
    public void freeDrive(double y, double x, double heading) {

        // 计算四个轮子的速度值
        leftFrontPower = (cos(heading) + sin(heading)) * (y + x);
        leftRearPower = (cos(heading) - sin(heading)) * (y - x);
        rightFrontPower = (cos(heading) - sin(heading)) *  (y - x);
        rightRearPower = (cos(heading) + sin(heading)) * (y + x);
